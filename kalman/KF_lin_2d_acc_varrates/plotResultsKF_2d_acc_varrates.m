...
subplot(3,2,[1 2]);
plot(signals.sig.x, signals.sig.y,'r', 'LineWidth', 2,'LineStyle','--','DisplayName','disturbed signal');hold on;
...
percentageImprovement = (rmse(signals.sig.x-clean.position.x)+rmse(signals.sig.y-clean.position.y))/ ...
...
subplot(3,2,3);
plot(ts.t, signals.sig.x,'r','LineWidth', 2,'DisplayName','disturbed x','LineStyle','--'); hold on;
...
subplot(3,2,4);
plot(ts.t, signals.sig.y,'r','LineWidth', 2,'LineStyle','--','DisplayName','disturbed y'); hold on;
