function plotResultsKF_2d(t,clean,signals,X_arr,velocity,add2title)
...
plot(X_arr(:, 1), X_arr(:, 3),'Color',[0 0 1],'LineWidth', 2,'DisplayName','KF result');
...
kfImprovement = num2str(round(rmse(X_arr(:,1)-clean.position.x) + rmse(X_arr(:,3)-clean.position.y),3));
...
plot(t, X_arr(:, 3),'Color',[0 0 1], 'LineWidth', 2,'DisplayName','KF result y');
grid on; grid minor; legend
percentageImprovement = rmse_improvement(clean.position.y,signals(1).sig.y,X_arr(:,3));
...
plot(t,  X_arr(:, 4),'Color',[0 0 1], 'LineWidth', 2,'DisplayName','KF result vel y');
grid on; grid minor; legend
percentageImprovement = rmse_improvement(clean.velocity.y,velocity.sig.y,X_arr(:,4));
