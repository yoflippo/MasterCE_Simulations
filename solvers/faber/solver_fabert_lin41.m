function location = solver_fabert_lin41(distances,anchorloc)
...
[loc,fval,exitflag,output] = linprog(f',A,b,Aeq,beq,lb,ub);
% Ax<b
...
%% ============================== Nested functions ==============================
    ...
    function result = ai(i)
        result = (ai_w - 2*anchorloc(i,1))*(1/distances(i));
    end

    function result = bi(i)
        result = (bi_w - 2*anchorloc(i,2))*(1/distances(i));
    end

    function result = ci(i)
        result = 1/(N*distances(i));
    end

    function result = di(i)
        result = ((di_w + (anchorloc(i,1)^2 + anchorloc(i,2)^2))*(1/distances(i))) - distances(i);
    end
