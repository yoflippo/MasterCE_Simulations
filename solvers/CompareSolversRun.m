...
figure
pdif = ProcentualDifference([results.error.fun1.Dist.noise{:}],[results.error.fun2.Dist.noise{:}]);
bar(pdif);
...
figure
pdif = ProcentualDifference([results.error.fun1.Pos.noise{:}],[results.error.fun2.Pos.noise{:}]);
bar(pdif);
title(['Sum error: ' num2str(round(sum(pdif),1)) ', mean error: ' num2str(round(mean(pdif),1))]);
ylabel('Procentual Error')
xlabel('Simulations'); grid on; grid minor;
